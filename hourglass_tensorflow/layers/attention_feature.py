...
class FeatureAttentionMechanism(Layer):
    ...
    def call(self, inputs: tf.Tensor, training: bool = True) -> tf.Tensor: # training = True
        ...
        _inputs =self.in_ln(inputs,training=training) # Batch normalization to the raw inputs
        _inputs = tf.clip_by_value(_inputs,-4,4) # Clip the input feats to avoid numerical instability
