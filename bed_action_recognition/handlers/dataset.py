...
class _bARTFDatasetHandler(_HTFHandler):
...
class bARTFDatasetHandler(_bARTFDatasetHandler):
    @property
    ...
    def _generate_seq_sets(self, sequences: Set[str]) -> ImageSetsType:
        # Sequences: set of paths to the sequences.
        ...
        if self.has_train:
            ...
            if self.has_test & self.has_validation:
                ...
                train, test = split_train_test(
                    sequences, self.ratio_train + self.ratio_validation,self.ratio_test
                )
                train, validation = split_train_test(
                    train, self.ratio_train / (self.ratio_train + self.ratio_validation)
                )
            elif self.has_validation:
                train, validation = split_train_test(sequences, self.ratio_train)
            elif self.has_test:
                train, test = split_train_test(sequences, self.ratio_train)
            else:
                train = sequences
        else:
            if self.has_test & self.has_validation:
                test, validation = split_train_test(sequences, self.ratio_test)
            elif self.has_test:
                test = sequences
            else:
    ...
    def _split_by_ratio(self) -> Tuple[HTFDataTypes, HTFDataTypes, HTFDataTypes]:
        ...
        sequences = self.engine.get_sequences(data=self.data, column=self.config.column_sequence)
        seq_train, seq_test, seq_validation = self._generate_seq_sets(sequences)
        # Save on metadata
        self._metadata.test_sequences= seq_test
        self._metadata.train_sequences = seq_train
        self._metadata.validation_sequences = seq_validation
        # Select Subsets within the main data collection
        test = self.engine.select_subset_from_images(
            data=self.data, image_set=seq_test, column=self.config.column_image
        )
        train = self.engine.select_subset_from_images(
            data=self.data, image_set=seq_train, column=self.config.column_image
        )
        validation = self.engine.select_subset_from_images(
            data=self.data, image_set=seq_validation, column=self.config.column_image
        )
        return train, test, validation
